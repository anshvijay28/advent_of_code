...
def solve(ids):
    ...
    for line in lines:
        # see if we can update curr_id
        data = line.split(" ")
        if isNum(data[3][1:]):
            curr_id = int(data[3][1:])
            if curr_id not in guards:
                guards[curr_id] = [0] * 60
            continue

        # if we are here then we are processing a sleep/wake up
        if data[3] == "asleep":
            sleep_time = data[1][:-1]
            continue 

        wake_time = data[1][:-1]
        wake_min = int(wake_time.split(":")[1])
        sleep_min = int(sleep_time.split(":")[1])
        min_slept = 0

        if wake_min >= sleep_min:
            min_slept = wake_min - sleep_min
        else:
            min_slept = (60 - sleep_min) + wake_min

        # mark the minutes this guard as slept
        for i in range(sleep_min, sleep_min+min_slept):
            guards[curr_id][i%60] += 1
            
            if guards[curr_id][i%60] > max_freq:
                max_freq = guards[curr_id][i%60]
                max_min = i%60
                guard = curr_id
        ...
        # guards_sleep[curr_id] += min_slept

        # if guards_sleep[curr_id] > max_min_slept:
        #     max_min_slept = guards_sleep[curr_id]
        #     sleepy_guard = curr_id

    # after gathering sleep data
    # mx, idx = 0, 0
    # for i in range(60):
    #     if guards[sleepy_guard][i] > mx:
    #         mx = guards[sleepy_guard][i]
    #         idx = i
